...
namespace eosio { namespace chain {
   ...
   template <typename F>
   void for_each_rocksdb_index_with_prefix(eosio::session::undo_stack<rocks_db_type>* kv_undo_stack, const eosio::session::shared_bytes& prefix_key, F&& function) {
      ...
      if (db.get<kv_db_config_object>().backing_store == backing_store_type::ROCKSDB && kv_undo_stack) {
         ...
         for_each_rocksdb_index_with_prefix(
            ...
            [&function](uint64_t contract, const char* key, std::size_t key_size, const char* value,
               ...
               kv_object_view row{name(contract), {{key + sizeof(contract), key + key_size}}, {{pp.value, pp.value + pp.value_size}}, pp.payer};
               function(row);
            });
   ...
   void combined_database::add_to_snapshot(
      ...
      eosio::chain::controller_index_set::walk_indices([this, &snapshot](auto utils) {
         ...
         snapshot->write_section<value_t>([utils, this](auto& section) {
            walk_index(utils, kv_undo_stack.get(), db, [this, &section](const auto& row) { section.add_row(row, db); });
         });
      });
   ...
   struct rocksdb_contract_table_writer {
      ...
      const eosio::session::undo_stack<rocks_db_type>* kv_undo_stack_;    // (possibly) REMOVE
      ...
      rocksdb_contract_table_writer(Section& s, const chainbase::database& db, const eosio::session::undo_stack<rocks_db_type>* undo_stack)
         : section_(s), db_(db), kv_undo_stack_(undo_stack){}
   ...
   void combined_database::add_contract_tables_to_snapshot(const snapshot_writer_ptr& snapshot) const {
      snapshot->write_section("contract_tables", [&db = db, kv_undo_stack = kv_undo_stack.get()](auto& section) {
         if (db.get<kv_db_config_object>().backing_store == backing_store_type::CHAINBASE) {
            ...
            auto prefix_key = eosio::session::make_shared_bytes(rocksdb_contract_db_prefix.data(),
                                                                rocksdb_contract_db_prefix.size());
            for_each_rocksdb_index_with_prefix(kv_undo_stack, prefix_key, writer);
         }
      });
   }
   ...
   template <typename Section>
   void rocksdb_read_contract_tables_from_snapshot(std::unique_ptr<rocks_db_type>& kv_database, chainbase::database& db,
                                                   Section& section) {
      using kv_batch = std::vector<std::pair<eosio::session::shared_bytes, eosio::session::shared_bytes> >;
      kv_batch            batch;
      bool                more     = !section.empty();
      ...
      while (more) {
         ...
         read_row(table_obj);
         auto put = [&batch, &table_obj](const eosio::session::shared_bytes& value, auto create_fun, auto&&... args) {
            auto key = backing_store::db_key_value_format::create_full_key(
         ...
         auto write_secondary_index = [&put, &read_row](auto index) {
            ...
            const char null = '\0';
            auto empty_payload = eosio::session::make_shared_bytes(&null, 1);
            unsigned_int       size;
            read_row(size);
         ...
         b1::chain_kv::bytes (*create_table_key)(name , name ) = backing_store::db_key_value_format::create_table_key;
