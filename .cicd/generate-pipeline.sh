...
for FILE in $(ls $CICD_DIR/platforms/$PLATFORM_TYPE); do
    ...
    if [[ $FILE_NAME =~ 'macos-10.14' ]]; then
      ...
      export ANKA_TEMPLATE_NAME='10.15.5_6C_14G_80G_HTT'
    else # Linux
...
EOF
  ...
  - label: "$(echo "$PLATFORM_JSON" | jq -r .ICON) $(echo "$PLATFORM_JSON" | jq -r .PLATFORM_NAME_FULL) - Build"
    command:
      - "git clone https://github.com/EOSIO/eos eos && cd eos && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT && git submodule update --init --recursive"
    ...
    plugins:
      - EOSIO/anka#v0.6.1:
          ...
          failover-registries:
            ...
            - 'registry_2'
          pre-commands: 
            - "rm -rf mac-anka-fleet; git clone git@github.com:EOSIO/mac-anka-fleet.git -b htt-gitupdate && cd mac-anka-fleet && . ./ensure-tag.bash -u 12 -r 25G -a '-n'"
...
cat <<EOF
  ...
  - label: ":darwin: macOS 10.14 - Package Builder"
    command:
      - "git clone https://github.com/EOSIO/eos eos && cd eos && $GIT_FETCH git checkout -f \$BUILDKITE_COMMIT"
